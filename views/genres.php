...
$count = DB::queryFirstField("SELECT COUNT(*) FROM genres WHERE %l", $where);
...
$genres = DB::query("SELECT *, (SELECT COUNT(*) FROM book_genres WHERE genre_id = genres.id) as books FROM genres WHERE %l ORDER BY %b $sort_by LIMIT %i OFFSET %i", $where, $order_by, $limit, $offset);
...
?>
    ...
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h1 class="h2"><i class="fas fa-theater-masks"></i> Genres</h1>
    ...
    <div class="row">
        <div class="col-sm-8"><h2>All <strong>Genres</strong></h2></div>
        <div class="col-sm-4">
        ...
        <table class="table table-striped">
            ...
            <tbody>
                <?php foreach ($genres as $genre): ?>
                    <tr class="cursor-pointer" onclick="window.location.href='/books?genre=<?= $genre['id'] ?>'">
                        <td><?= $genre['id'] ?></td>
                        <td><?= $genre['name'] ?></td>
                        <td><?= $genre['books'] ?></td>
                    </tr>
